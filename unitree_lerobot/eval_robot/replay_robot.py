...
unitree_idx = [0, 2, 4, 6, 8, 10, 12, 1, 3, 5, 7, 9, 11, 13, 16, 22, 26, 15, 21, 14, 20, 19, 25, 27, 17, 23, 18, 24]
...
def replay_main(cfg: EvalRealConfig):
    ...
    # adjust index
    init_left_arm_pose = init_left_arm_pose[unitree_idx[:14]]

    user_input = input("Please enter the start signal (enter 's' to start the subsequent program):")
    if user_input.lower() == "s":
        while True:
            # "The initial positions of the robot's arm and fingers take the initial positions during data recording."
            logger_mp.info("Smoothly moving robot to starting pose...")
            
            current_pose = arm_ctrl.get_current_dual_arm_q()
            
            steps = 30
            for step in range(steps + 1):
                alpha = step / steps  # 0 to 1
                target_pose = current_pose * (1 - alpha) + init_left_arm_pose * alpha
                tau = arm_ik.solve_tau(target_pose)  # based on current interpolated position
                arm_ctrl.ctrl_dual_arm(target_pose, tau)
                time.sleep(1.0 / cfg.frequency)  # use the same frequency
            
            logger_mp.info("Robot reached starting pose smoothly.")
            time.sleep(1)
            for idx in range(dataset.num_frames):
                loop_start_time = time.perf_counter()

                left_ee_state = right_ee_state = np.array([])
                action_np_i4h_order = actions[idx]["action"].numpy()
                action_np = action_np_i4h_order[unitree_idx]
